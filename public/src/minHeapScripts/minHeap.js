...
export default class MinHeap {
  ...
  deepCopy() {
    ...
    let newTree = new MinHeap(newPos, this.nodeSize)
    ...
    if(this.root !== undefined) {
      ...
      newTree.array = MinHeap.buildArray(newTree.root, newTree.size)
    }
  ...
  static buildArray(root, size) {
    ...
    MinHeap.buildArrayAux(root, array, 0)
    return array
  }
  ...
  static buildArrayAux(node, array, index) {
    ...
    if(node.left !== undefined) {
      MinHeap.buildArrayAux(node.left, array, 2 * index + 1)
    }
    if(node.right !== undefined) {
      MinHeap.buildArrayAux(node.right, array, 2 * index + 2)
    }
  }
